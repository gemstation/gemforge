...
import { GemforgeConfig, cli, exec, expect, fileExists, getTestDataFolderPath, loadDiamondContract, loadFile, loadJsonFile, loadWallet, removeFile, sendTx, updateConfigFile, updateFile, writeFile } from './utils.js'
...
module.exports = {
  ...
  etherscan: {
    ...
    it('tries to verify the contracts', async () => {
      expect(cli('build', { cwd, verbose: false }).success).to.be.true
      const ret = cli('deploy', 'local', { cwd, verbose: false })
      expect(ret.success).to.be.false // haven't been able to get a mock verification server to work yet, so for now we do this
      ...
      // check for call
      expect(ret.output).to.contain(checkFor)
    })
  })
}
